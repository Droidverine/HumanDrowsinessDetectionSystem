...
db.child("users").child(args["email"]).update({"status": "Running"})
db.child("users").child(args["email"]).update({"longitude": "73.0.174"})
db.child("users").child(args["email"]).update({"latitude": "19.0440"})
		...
		if ear < EYE_AR_THRESH:
			...
			if COUNTER >= EYE_AR_CONSEC_FRAMES:
				...
				if not ALARM_ON:
					...
					if COUNTERCHECK>= 3:
						COUNTERCHECK=0
						db.child("users").child(args["email"]).child("logs").push({
                        "msghead": "Drowsiness Alert",
                        "msgtext": "Nerul",
...
db.child("users").child(args["email"]).update({"status": "Stopped"})
vs.stop()
